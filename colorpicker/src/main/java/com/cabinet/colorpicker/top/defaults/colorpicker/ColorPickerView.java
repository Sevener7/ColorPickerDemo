...
public class ColorPickerView extends LinearLayout implements ColorObservable {
    ...
    private ColorChoiceRectView colorChoiceRectView;
...
//        setEnabledChoice(true);
        setEnabledAlphaNew(true);
//        if (colorChoiceRectView == null) {
//            colorChoiceRectView = new ColorChoiceRectView(getContext());
//            LayoutParams layoutParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 2*sliderHeight);
//            layoutParams.topMargin = sliderMargin;
//            layoutParams.bottomMargin = sliderMargin;
//            addView(colorChoiceRectView, layoutParams);
//
//        }
    ...
    public void setEnabledAlphaNew(boolean enable) {
        if (enable) {
            if (colorChoiceRectView == null) {
                colorChoiceRectView = new ColorChoiceRectView(getContext());
                LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, 2*sliderHeight);
                params.topMargin = sliderMargin;
                addView(colorChoiceRectView, params);
            }

            ColorObservable bindTo = alphaSliderView;
            if (bindTo == null) {
            ...
            colorChoiceRectView.bind(bindTo);
            updateObservableOnDuty();
        } else {
            if (colorChoiceRectView != null) {
                colorChoiceRectView.unbind();
                removeView(colorChoiceRectView);
                colorChoiceRectView = null;
            }
            updateObservableOnDuty();
        }
    }

    public void setEnabledChoice(boolean enable) {
        if (enable) {
            if (colorChoiceRectView == null) {
                colorChoiceRectView = new ColorChoiceRectView(getContext());
                LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, 2 * sliderHeight);
                params.topMargin = sliderMargin;
                addView(colorChoiceRectView, -1, params);
            }
            colorChoiceRectView.bind(colorWheelView);
            updateObservableOnDuty();
        } else {
            if (colorChoiceRectView != null) {
                colorChoiceRectView.unbind();
...
//        }
    ...
    private void updateObservableOnDuty() {
        ...
        if (colorChoiceRectView != null) {
            colorChoiceRectView.setOnlyUpdateOnTouchEventUp(false);
        }
        if (brightnessSliderView == null && alphaSliderView == null && colorChoiceRectView == null) {
            observableOnDuty = colorWheelView;
            colorWheelView.setOnlyUpdateOnTouchEventUp(onlyUpdateOnTouchEventUp);
        } else {
            if(colorChoiceRectView !=null){
                observableOnDuty = colorChoiceRectView;
                colorChoiceRectView.setOnlyUpdateOnTouchEventUp(onlyUpdateOnTouchEventUp);
            } else if (alphaSliderView != null) {
                observableOnDuty = alphaSliderView;
                alphaSliderView.setOnlyUpdateOnTouchEventUp(onlyUpdateOnTouchEventUp);
            } else if (brightnessSliderView != null) {
